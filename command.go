...
// Find returns a Source that walks the tree rooted at root on the configured
...
func Find(root gloo.File, opts ...any) gloo.Source[[]byte] {
	return findSource{root: root, cfg: fold(opts)}
}
...
func (s findSource) Stream(ctx context.Context) gloo.Stream[[]byte] {
	return gloo.Generate(ctx, func(_ context.Context, send func([]byte) bool, sendErr func(error)) {
		if err := afero.Walk(s.cfg.fs, string(s.root), s.visit(send)); err != nil {
			sendErr(err)
		}
	})
}
...
func (s findSource) withinDepth(path string) bool {
	...
	return depth(s.root, gloo.File(path)) <= int(s.cfg.maxDepth)
}
...
func (t FindType) accepts(info fs.FileInfo) bool {
	switch t {
	case FindTypeFile:
		return !info.IsDir()
	case FindTypeDir:
...
func (n FindName) matches(base string) bool {
	if n == "" {
		return true
	}
...
func depth(root, path gloo.File) int {
	rel := strings.TrimPrefix(string(path), string(root))
