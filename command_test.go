...
func TestFind_WalksWholeTree(t *testing.T) {
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}))
...
func TestFind_NameGlob(t *testing.T) {
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindName("*.go")))
	assertPaths(t, got, []string{"/root/keep.go", "/root/sub/deep/c.go"})
}

func TestFind_NameGlobNoMatch(t *testing.T) {
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindName("*.md")))
...
func TestFind_MalformedGlobMatchesNothing(t *testing.T) {
	...
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindName("[")))
	assertPaths(t, got, nil)
}

func TestFind_TypeFile(t *testing.T) {
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindTypeFile))
	assertPaths(t, got, []string{
...
func TestFind_TypeDir(t *testing.T) {
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindTypeDir))
...
func TestFind_TypeUnknownIsUnfiltered(t *testing.T) {
	...
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindType("x")))
	assertPaths(t, got, []string{
		"/root", "/root/a.txt", "/root/keep.go",
		"/root/sub", "/root/sub/b.txt", "/root/sub/deep", "/root/sub/deep/c.go",
	})
...
func TestFind_MaxDepthZero(t *testing.T) {
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindMaxDepth(0)))
	assertPaths(t, got, []string{"/root"})
}

func TestFind_MaxDepthOne(t *testing.T) {
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindMaxDepth(1)))
...
func TestFind_MaxDepthPrunesDeepFile(t *testing.T) {
	...
	got := run(t, command.Find("/root", command.FindFs{Fs: tree(t)}, command.FindMaxDepth(2)))
	assertPaths(t, got, []string{
		"/root", "/root/a.txt", "/root/keep.go",
...
func TestFind_TypeAndMaxDepthCompose(t *testing.T) {
	got := run(t, command.Find("/root",
		command.FindFs{Fs: tree(t)}, command.FindTypeFile, command.FindMaxDepth(1)))
	assertPaths(t, got, []string{"/root/a.txt", "/root/keep.go"})
}

func TestFind_NameAndTypeCompose(t *testing.T) {
	got := run(t, command.Find("/root",
		command.FindFs{Fs: tree(t)}, command.FindName("*.go"), command.FindTypeFile))
	assertPaths(t, got, []string{"/root/keep.go", "/root/sub/deep/c.go"})
}

func TestFind_MissingRootIsAnError(t *testing.T) {
	_, err := gloo.Collect(context.Background(),
		command.Find("/nope", command.FindFs{Fs: afero.NewMemMapFs()}).Stream(context.Background()))
	if err == nil {
...
func TestFind_DeepEntryStatErrorIsSkipped(t *testing.T) {
	...
	got := run(t, command.Find("/root", command.FindFs{Fs: fs}))
	for _, p := range got {
		if p == "/root/sub" {
...
func TestFind_RootStatErrorIsFatal(t *testing.T) {
	...
	_, err := gloo.Collect(context.Background(),
		command.Find("/root", command.FindFs{Fs: fs}).Stream(context.Background()))
	if err == nil {
